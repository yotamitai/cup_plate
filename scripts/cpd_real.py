...
import sys
places = {
	"table_side2":[1.21, 0.9],
	"table_side1":[1.15, 2.66],
	"plate_pile" :[2.75, 1.0],
	"cup_stack"  :[2.75,2.47],
}
location = '/home/st.technion.ac.il/yotama/workspace'
...
def simple_move_client(x_pos = 0.0, y_pos = 0.0, robot = ''):
	...
	print "start wait_for_server"
	# Waits until the action server is online
	client.wait_for_server()
	print "start send_goal"
	# send goal to client
	client.send_goal(goal)
	print "start wait_for_result"
	# wait for result
